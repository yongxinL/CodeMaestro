...
**Note:** The `.gitignore` generated by `setup.sh` already includes these patterns, and Docker respects `.dockerignore` first, then `.gitignore`.
...
A: Re-run `setup.sh` from the CodeMaestro installation to restore framework files. Your user documentation in `docs/` won't be affected.

**Q: Should framework files be in git history?**
A: Yes during development. The `.gitignore` generated by `setup.sh` excludes them, but they're present in your working directory. Only remove them before final release.
